...
public protocol Record: Identifiable, Decodable, EncodableWithConfiguration, Sendable, Equatable, Hashable where EncodingConfiguration == PocketBase.EncodingConfiguration {
    ...
    /// Extracts any pending file uploads from this record's FileValue fields.
    ///
    /// This method inspects all `@FileField` properties that use `FileValue` type
    /// and returns a payload containing any pending uploads (files that haven't
    /// been uploaded to the server yet).
    ///
    /// For records using the legacy `RecordFile` type, this returns an empty dictionary.
    ///
    /// This property is automatically generated by the `@BaseCollection` or
    /// `@AuthCollection` macros.
