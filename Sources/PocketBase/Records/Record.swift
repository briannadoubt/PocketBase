...
public protocol Record: Identifiable, Decodable, EncodableWithConfiguration, Sendable, Equatable, Hashable where EncodingConfiguration == PocketBase.EncodingConfiguration {
    ...
    ///
    /// File fields are marked with `@File` and store filename strings.
    ...
    ///
    /// This method inspects all `@File` properties and returns a payload
    ...
    /// Extracts file field values preserving order for mixed existing/pending arrays.
    ///
    /// This method inspects all `@File` properties and returns a dictionary
    /// mapping field names to arrays of `FileFieldEntry` values that preserve the
    /// original order of existing and pending files. This is critical for updates
    /// where array order matters.
    ///
    /// This method is automatically generated by the `@BaseCollection` or
    /// `@AuthCollection` macros.
...
/// Represents a single entry in a file field, preserving order information.
public enum FileFieldEntry: Sendable {
    /// An existing file on the server (just the filename).
    case existing(String)
    /// A pending file to upload.
